...
class PublishToConfluenceTask extends DefaultTask {
    static final CDATA_PLACEHOLDER_START = '<cdata-placeholder>'
    static final CDATA_PLACEHOLDER_END = '</cdata-placeholder>'
    ...
    @TaskAction
    void run() {
        def docDir = project.property("docDir") as String
        def confluenceConfigFile = project.property("confluenceConfigFile") as String

        // configuration
        logger.info("docDir: ${docDir}")
        logger.info("confluenceConfigFile: ${confluenceConfigFile}")
    ...
    def uploadAttachment(pageId, String url, String fileName, String note) {
        def is
        def localHash
    ...
    def pushToConfluence(pageTitle, pageBody, parentId, anchors, pageAnchors) {
        ...
        def localPage = parseBody(pageBody, anchors, pageAnchors)
        ...
        if (page) {
            ...
            //create a page
            logger.info("body: ${request}, headers: ${headers}")
            trythis {
    ...
    def pushPages(pages, anchors, pageAnchors) {
        pages.each { page ->
            println page.title
